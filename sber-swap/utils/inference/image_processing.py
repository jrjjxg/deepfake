...
def normalize_and_torch(image: np.ndarray, device=None) -> torch.tensor:
    """
    Normalize image and transform to torch
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    image = torch.tensor(image.copy(), dtype=torch.float32).to(device)
    if image.max() > 1.:
        image = image/255.
...
def normalize_and_torch_batch(frames: np.ndarray, device=None) -> torch.tensor:
    """
    Normalize batch images and transform to torch
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    batch_frames = torch.from_numpy(frames.copy()).to(device)
