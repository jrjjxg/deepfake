...
def transform_target_to_torch(resized_frs: np.ndarray, device, half=True) -> torch.tensor:
    """
...
def model_inference(full_frames: List[np.ndarray],
    ...
    for idx, (crop_frames, tfm_array, source_embed) in enumerate(zip(crop_frames_list, tfm_array_list, source_embeds)):
        ...
        target_batch_rs = transform_target_to_torch(resized_frs, device, half=use_half)
