"""
运动特征检测器 - Celeb-DF-v2 推理测评脚本
Motion Feature Detector - Celeb-DF-v2 Inference Script
...
使用方法：
1. 上传训练好的模型到 Kaggle: motion_feature_model.joblib
2. 添加 Celeb-DF-v2 数据集
3. 运行脚本
"""
...
class Config:
    ...
    MODEL_PATH = "motion_feature_model.joblib"  # Upload this file
...
# ========== Evaluation ==========
def evaluate_model(y_true, y_pred, y_prob, split_name: str = "Test Set"):
...
def main():
    print("\n" + "="*70)
    print("Motion Feature Detector - Celeb-DF-v2 Inference")
    print("="*70 + "\n")
    
    # Check model file
    if not os.path.exists(Config.MODEL_PATH):
        print(f"ERROR: Model file not found: {Config.MODEL_PATH}")
        print("Please upload the trained model to Kaggle.")
        sys.exit(1)
    
    # Load model
    print("[Step 1/4] Loading trained model...")
    model = joblib.load(Config.MODEL_PATH)
    print(f"  Model loaded from: {Config.MODEL_PATH}")
    
    # Collect videos
    print("\n[Step 2/4] Collecting videos from Celeb-DF-v2...")
    ...
    # Extract features
    print("\n[Step 3/4] Extracting motion features...")
    ...
    with mp_face_mesh.FaceMesh(
        ...
        for idx, (video_id, frame_paths, label) in enumerate(tqdm(video_list, desc="Processing")):
    ...
    print(f"\n  Final: {len(X_test)} samples extracted ({success_count} success, {fail_count} failed)")
    print(f"  Feature dimensions: {X_test.shape[1]}")
    
    # Predict
    print("\n[Step 4/4] Evaluating on Celeb-DF-v2...")
    ...
    # Evaluate
    metrics = evaluate_model(y_test, y_pred, y_prob, "Celeb-DF-v2 Test Set")
