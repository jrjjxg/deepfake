...
function showPreview(file, preview, drop) {
    ...
    setTimeout(() => preview.style.opacity = '1', 10); // Simple fade in
...
fetch('/api/models')
    ...
    .catch((e) => {
        console.error(e);
        document.getElementById('model-options-grid').innerHTML = '<div style="color:red">Loading failed</div>';
    });
        ...
        if (data.error) {
            ...
            result.innerHTML = `
                <div class="result-header">
                    <span class="result-badge ${colorClass}">${isFake ? 'DETECTED FAKE' : 'LEGITIMATE IMAGE'}</span>
                </div>
                ...
                ${data.note ? '<div class="result-detail" style="margin-top:16px">' + data.note + '</div>' : ''}
            `;
        }
    } catch (e) {
        result.innerHTML = '<span class="error">请求失败，请检查服务是否正常</span>';
    }
...
async function runSwap() {
    ...
    result.className = 'result show';
    result.innerHTML = '<div style="text-align:center; color:#6b7280;">正在生成换脸图像，请稍候...</div>';
    ...
    try {
        ...
        if (data.error) {
            ...
            result.innerHTML = `
                ...
                ${data.note ? '<div class="result-detail" style="margin-top:12px">' + data.note + '</div>' : ''}
            `;
...
async function runAutoDetect(base64Image, resultContainer) {
    ...
    resultContainer.className = 'result detection-result show';
    resultContainer.innerHTML = '<div class="result-title">伪造检测分析</div><div style="color:#6b7280;">正在分析换脸结果...</div>';
    ...
    try {
        ...
        if (data.error) {
            resultContainer.innerHTML = '<div class="result-title">伪造检测分析</div><span class="error">检测错误: ' + data.error + '</span>';
        } else {
            const fakeProb = data.fake * 100;
            const realProb = data.real * 100;
            const isFake = data.fake > 0.5;
        ...
        resultContainer.innerHTML = '<div class="result-title">伪造检测分析</div><span class="error">检测请求失败</span>';
    }
}
...
async function generateVisualization(file, modelId) {
    ...
    try {
        ...
        visDescription.innerHTML = `
            <span class="vis-type-badge ${typeBadgeClass}">${data.detector_type || 'unknown'}</span>
        ...
        if (visCount === 0) {
            visGrid.innerHTML = '<div style="color:#6b7280; text-align:center; padding:20px;">暂无可视化结果</div>';
            return;
        }
        ...
        visGrid.innerHTML = '<div class="error">可视化请求失败，请检查服务状态</div>';
    }
}
